...
curl -N http://127.0.0.1:8888/both/ \
  ...
  -d '{"jsonrpc":"2.0","id":"1","method":"SendStreamingMessage","params":{"message":{"messageId":"msg-1","role":"ROLE_USER","parts":[{"text":"check the total number of security alarms"}]}}}'
```
...
curl http://127.0.0.1:8888/both/ \
  -H 'X-HW-ID: hw-id-003' -H 'X-HW-APPKEY: hw-key-003' -H 'Content-Type: application/json' \
  -d '{"jsonrpc":"2.0","id":"2","method":"SendMessage","params":{"message":{"messageId":"msg-2","role":"ROLE_USER","parts":[{"text":"check pedestrian flow at each entrance"}]}}}'
```
